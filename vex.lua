...
local SearchClass = false
local accurateSearch = false
local nodes = {}
local selection
		...
		local function main()
			...
			Explorer.ShowRightClick = function(MousePos)
				...
				for i = 1, #sList do
					...
					if not presentClasses.CanClone or not presentClasses.NotCreatable or not presentClasses.isViableDecompileScript then
						presentClasses.isViableDecompileScript = env.isViableDecompileScript(obj)
						presentClasses.NotCreatable = apiClasses[obj.ClassName].Tags.NotCreatable or false
						presentClasses.CanClone = not apiClasses[obj.ClassName].Tags.Service and obj.Archivable == true
					end
					if not class then
						class = obj.ClassName
						node.Class = class
				...
				--local ui = {"TextLabel", "TextBox", "ScreenGui", "Frame"}

				context:AddRegistered("CUT")
				context:AddRegistered("COPY")
				context:AddRegistered("PASTE", emptyClipboard)
				context:AddRegistered("DUPLICATE", not presentClasses.CanClone)
				context:AddRegistered("DELETE")
				context:AddRegistered("DELETE_CHILDREN")
				...
				context:AddRegistered("SELECT_ALL")
				...
				--context:AddRegistered("VIEW_SIGNALS")
			    ...
			    if not presentClasses.NotCreatable and Main.Testing then
				    context:AddRegistered("GUI_TO_CODE")
				end
			...
			Explorer.InitRightClick = function()
				...
				context:Register("PASTE",{Name = "粘贴", IconMap = Explorer.MiscIcons, Icon = "Paste", DisabledIcon = "Paste_Disabled", Shortcut = "Ctrl+Shift+V", OnClick = function()
					local sList = selection.List
					local newSelection = {}
					local count = 1
					for i = 1,#sList do
				...
				context:Register("DELETE_CHILDREN",{Name = "删除子物体", IconMap = Explorer.MiscIcons, Icon = "Delete", DisabledIcon = "Delete_Disabled", Shortcut = "Del", OnClick = function()
					local sList = selection.List
					for i = 1,#sList do
				...
				context:Register("RENAME",{Name = "重命令", IconMap = Explorer.MiscIcons, Icon = "Rename", DisabledIcon = "Rename_Disabled", Shortcut = "F2", OnClick = function()
					local sList = selection.List
					if sList[1] then
				...
				context:Register("UNGROUP",{Name = "取消组合", IconMap = Explorer.MiscIcons, Icon = "Ungroup", DisabledIcon = "Ungroup_Disabled", Shortcut = "Ctrl+U", OnClick = function()
					local newSelection = {}
					local count = 1
					local isa = game.IsA
				...
				context:Register("SAVE_INST",{Name = "保存为文件", IconMap = Explorer.MiscIcons, Icon = "Save", OnClick = function()
				     local sList = selection.List
				     Lib.SaveAsPrompt(sList[1].Obj)
				end})

				context:Register("SELECT_CHILDREN",{Name = "选择所有子物体", IconMap = Explorer.MiscIcons, Icon = "SelectChildren", DisabledIcon = "SelectChildren_Disabled", OnClick = function()
					local newSelection = {}
					local count = 1
					local sList = selection.List
				...
				context:Register("SELECT_ALL",{Name = "选择所有", IconMap = Explorer.MiscIcons, Icon = "SelectChildren", DisabledIcon = "SelectChildren_Disabled", OnClick = function()
					local sList = selection.List
					local newSelection = {}
					local count = 1
					local function expand(explorerNode)
						--expanded[explorerNode] = true
    					local inst = explorerNode
    					if inst then
        					newSelection[count] = inst
        					count += 1
    					end
    					for i = 1, #explorerNode do
        					expand(explorerNode[i])
   						end
					end
				...
				context:Register("JUMP_TO_PARENT",{Name = "跳到父体", IconMap = Explorer.MiscIcons, Icon = "JumpToParent", OnClick = function()
					local newSelection = {}
					local count = 1
					local sList = selection.List
				...
				context:Register("TELEPORT_TO",{Name = "传送", IconMap = Explorer.MiscIcons, Icon = "TeleportTo", OnClick = function()
					local sList = selection.List
				...
				local OldAnimation
				context:Register("LOAD_ANIMATION",{Name = "加载动作", IconMap = Explorer.MiscIcons, Icon = "Play", OnClick = function()
					local sList = selection.List
				...
				context:Register("STOP_ANIMATION",{Name = "停止动作", IconMap = Explorer.MiscIcons, Icon = "Pause", OnClick = function()
					local sList = selection.List
				...
				context:Register("CLEAR_SEARCH_AND_JUMP_TO",{Name = "清除搜索和跳到", OnClick = function()
					local newSelection = {}
					local count = 1
					local sList = selection.List
				...
				local clth = function(str)
					str = str:gsub("game:GetService%(\"Workspace\"%)", "workspace")

					local escaped = plr.Name:gsub("([%.%+%-%*%?%[%]%^%$%(%)%%])", "%%%1")
					str = str:gsub("workspace%." .. escaped, "game:GetService(\"Players\").LocalPlayer.Character")

					str = str:gsub("game:GetService%(\"Players\"%)%." .. escaped,
				...
				context:Register("INSERT_OBJECT",{Name = "插入物体", IconMap = Explorer.MiscIcons, Icon = "InsertObject", OnClick = function()
                ...
                context:Register("VIEW_SIGNALS",{Name = "复制所有可连接的事件", OnClick = function()
                	local signals = Explorer.GetConnections(selection.List[1].Obj)
                	local code = "{"
                ...
                context:Register("GUI_TO_CODE",{Name = "转代码", IconMap = Explorer.ClassIcons, Icon = 18, OnClick = function()
                    ...
                    for i = 1, #sList do
                        local function iterative(obj)
							local node = obj
                        	local cnode
							if API.Classes[node.ClassName].Tags and not API.Classes[node.ClassName].Tags.NotCreatable then
								cnode = Instance.new(node.ClassName)
							else
								return
							end
							...
							local function toLiteral(v)
								local t = typeof(v)
								if t == "string" then return string.format("[=[%s]=]", v) end
								--if t == "Content" then return '"' .. tostring(v) .. '"' end
								if t == "boolean" then return tostring(v) end
								if t == "number"  then return tostring(v) end
								if t == "EnumItem" then return tostring(v) end
								if t == "Instance" then
									return clth(Explorer.GetInstancePath(v))
   	   							end
								if t == "Vector2" then
								...
								if t == "Font" then
									return string.format("Font.new(%q, %s, %s)",
										v.Family, toLiteral(v.Weight), toLiteral(v.Style))
								end
								if t == "UDim2" then
								...
								if t == "Color3" then
									return string.format("Color3.fromRGB(%s, %s, %s)",
								...
								if t == "ColorSequence" then
									...
									for i, kp in ipairs(kps) do
										tmp[i] = string.format("ColorSequenceKeypoint.new(%s, %s)",
											kp.Time, toLiteral(kp.Value))
									end
								...
								if t == "NumberSequence" then
									...
									local tmp = table.create(#kps)
									for i, kp in ipairs(kps) do
										tmp[i] = string.format("NumberSequenceKeypoint.new(%s, %s, %s)",
                		    ...
                		    local function getProps(obj,classData)
                	 	       local result = {}
                     			local counts = 1
			      			   	local props = classData.Properties
			         			for i = 1,#props do
				        			local prop = props[i]
									local s,r = pcall(function() return obj[prop.Name] end)
									if s then
					     	  			result[counts] = {PN = prop.Name, PV = r, tags = prop.Tags, PT = typeof(r)}
					     	  			counts += 1
									end
								end
				       			return result
				   			end
				    	   	local apiclass = API.Classes[node.ClassName]
							local props = {}
							while apiclass do
								table.insert(props, getProps(node, apiclass))
...
%s%s = Instance.new("%s")]]
				...
				context:Register("FIRE_TOUCHTRANSMITTER",{Name = "触发", IconMap = Explorer.ClassIcons, Icon = 37, OnClick = function()
				...
				context:Register("FIRE_CLICKDETECTOR",{Name = "触发", IconMap = Explorer.ClassIcons, Icon = 41, OnClick = function()
					local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
					if not hrp then return end
					for _, v in ipairs(selection.List) do if v.Obj and v.Obj:IsA("ClickDetector") then fireclickdetector(v.Obj) end end
				end})

				context:Register("FIRE_PROXIMITYPROMPT",{Name = "触发", IconMap = Explorer.ClassIcons, Icon = 124, OnClick = function()
					local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
					if not hrp then return end
				...
				context:Register("SAVE_SCRIPT",{Name = "保存脚本", IconMap = Explorer.MiscIcons, Icon = "Save", OnClick = function()
					for _, v in next, selection.List do
						if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
							...
							if not success or not source then source = ("-- Vex - %s failed to decompile %s"):format(env.executor, v.Obj.ClassName) end
				...
				context:Register("SELECT_CHARACTER",{Name = "选择躯体", IconMap = Explorer.ClassIcons, Icon = 9, OnClick = function()
					local newSelection = {}
					local count = 1
					local sList = selection.List
					local isa = game.IsA
				...
				context:Register("SELECT_ALL_CHARACTERS",{Name = "选择所有躯体", IconMap = Explorer.ClassIcons, Icon = 2, OnClick = function()
					local newSelection = {}
					local sList = selection.List
				...
				context:Register("REFRESH_NIL",{Name = "刷新", OnClick = function()
					Explorer.RefreshNilInstances()
				end})

				context:Register("HIDE_NIL",{Name = "隐藏", OnClick = function()
		...
		for obj in next,newNilRoots do
			...
			Explorer.GetConnections = function(Obj)
				--print(API.Classes.Workspace.Events[1].Name)
				local events = {}
				for _, event in pairs(API.Classes[Obj.ClassName].Events) do
					table.insert(events, event.Name)
				end
				--print(API.Classes.Instance.Events)
				--print(API.Classes.Instance.Events[1])
				--print(API.Classes.Instance.Events[1].Name)
				--print(API.Classes.Instance.Events[1].Tags)
				for _, generalevent in pairs(API.Classes.Instance.Events) do
				    if not generalevent.Tags.Deprecated then
				        table.insert(events, generalevent.Name)
				    end
				end
				return events
			end
			...
			Explorer.DoSearch = function(query)
				...
				local defaultSearch = (function(Obj, str)
					if not accurateSearch then
						return (SearchClass and Obj.ClassName:lower() or Obj.Name:lower()):find(str, 1, true)
					else
						return (SearchClass and Obj.ClassName or Obj.Name) == str
					end
				end)
				...
				local function Search(query)
					...
					local lower = (accurateSearch and query or query:lower())
		...
		--[=[if #query > 0 then
			...
			Explorer.UpdateSelectionVisuals = function()
				...
				local PartSelectionBox = Explorer.PartSelectionBoxVisual
				local isa = game.IsA
				local clone = game.Clone
				if not holder or not PartSelectionBox then
					...
					PartSelectionBox = Instance.new("ScreenGui")
					local function GetDeepestDescendant(parent)
                    	local deepest = nil
						local maxDepth = -1

						local function recurse(instance, depth)
                    ...
                    local folder2 = game.Players.LocalPlayer.PlayerGui
                    if folder and folder2 then
	                    local deepest, depth = GetDeepestDescendant(folder)
	                    local deepest2, depth2 = GetDeepestDescendant(folder2)
	                	if depth2 > depth and deepest2:FindFirstAncestor(game.Players.LocalPlayer.Name) then
	                   		PartSelectionBox.Parent = deepest2
	                    elseif deepest2:FindFirstAncestor(game.Players.LocalPlayer.Name) then
	                    	PartSelectionBox.Parent = deepest
	                    end
	                   	--print(deepest:GetFullName())
                    end
					Explorer.PartSelectionBoxVisual = PartSelectionBox
				...
				PartSelectionBox:ClearAllChildren()
				...
				for i = 1,#sList do
					...
					if node ~= workspaceNode then
						...
						elseif isa(obj,"PVInstance") and partEnabled then
							...
							newBox.Adornee = obj
							newBox.Parent = PartSelectionBox
							boxCount = boxCount + 1
						end
					end
				end
			...
			Properties.GetIndexableProps = function(obj,classData)
				...
				for i = 1,#props do
					...
					if not ignoreProps[prop.Name] then
						local s,r = pcall(function() return obj[prop.Name] end)
						--print(tostring(prop.Name) .. ": " .. tostring(r))
						if s then
							result[count] = prop
							count = count + 1
						end
			...
			Properties.DisplayAddAttributeWindow = function(editAttr)
				...
				if not win then
					...
					saveButton.OnClick:Connect(function()
						...
						if #name > 100 then
							errorLabel.Text = "名字超过100个字符"
							return
						elseif name:sub(1,3) == "RBX" then
							errorLabel.Text = "名字RBX开头'"
							return
						end
					...
					if prop then
						...
						if prop.SpecialRow then
							...
							if prop.CategoryName then
								...
								toggleAttributes.Text = (showingAttrs and "[设置开启]" or "[设置关闭]")
		...
		local function main()
			...
			ScriptViewer.ViewScript = function(scr)
				...
				if not success or not source then
				    if not env.decompile then
				        source, PreviousScr = ("-- Vex - %s 反编译失败 注入器没有反编译函数"):format(env.executor), nil
				    elseif (scr.ClassName == "Script" and (scr.RunContext == Enum.RunContext.Server)) or not scr:IsA("LocalScript") then
				        source, PreviousScr = ("-- Vex - %s 反编译失败 尝试反编译服务器脚本"):format(env.executor), nil
				    else
				        source, PreviousScr = ("-- Vex - %s 反编译失败 %s"):format(env.executor, scr.ClassName), nil   
				    end
				else
				    PreviousScr = scr
				    local decompileScr = source
				    source = "--[[\n脚本路径: "..Explorer.GetInstancePath(scr) .. "\n"
				    source = source .. "注入器: "..env.executor .. "\n]]\n\n" .. decompileScr
				end
...
--[[
	...
	Container for functions and classes
		...
		local function main()
			...
			Lib.LoadHuiFolder = function()
			    local huiguis = env.gethui()
			    local folder = Instance.new("Folder", game)
			    folder.Name = "Hidden Guis"
			    for i,v in pairs(huiguis:GetChildren()) do
			        local new = v:Clone()
			        new.Parent = folder
			    end
			end
			...
			Lib.SaveAsPrompt = function(Obj)		
				...
				if not win then
					...
					saveButton.OnClick:Connect(function()
						local filename = (nameBox:GetText() ~= "" and nameBox:GetText()) or filename
						currentextension = filename:match("%.([^%.]+)$") or ""
						filename = filename:gsub("%.[^.]+$", "") .. currentextension
						local s,r = pcall(env.saveinstance,Obj,
						{
							FilePath = filename,
							Decompile = true,
								...
								releaseEvent = service.UserInputService.InputEnded:Connect(function(input)
									if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
										...
										if alignInsertSide then
											...
											--static.ToggleSide(alignInsertSide)
										end
									end
								end)
			...
			Lib.ColorPicker = (function() -- TODO: Convert to newer class model
				...
				local function new()
					...
					local guiContents = create({
						...
						{24,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Title",Parent={23},Size=UDim2.new(1,0,0,20),Text="自定义颜色",TextColor3=Color3.new(0.86274516582489,0.86274516582489,0.86274516582489),TextSize=14,TextXAlignment=0,}},
	...
	Save Instance Module
		...
		local function main()
			...
			SaveInstance.Init = function()
				...
				local Decompile = AddCheckbox("反编译脚本 (本地脚本和模块脚本)", SaveInstanceArgs.Decompile)
				...
				local decompileThread = AddTextbox("反编译最大进程", "3", 15)
				...
				local NilObj = AddCheckbox("保存Parent为nil的实例", SaveInstanceArgs.NilInstances)
				NilObj.OnInput:Connect(function()
					SaveInstanceArgs.NilInstances = NilObj.Toggled
				end)

				local RemovePlayerChar = AddCheckbox("移除玩家躯体", SaveInstanceArgs.RemovePlayerCharacters)
				...
				local IgnoreDefaultProps = AddCheckbox("忽略基础属性", SaveInstanceArgs.IgnoreDefaultProps)
				...
				FilenameTextBox.TextBox.Text = FilenameTextBox.TextBox.Text:gsub("{TIMESTAMP}", os.date("%d-%m-%Y_%H-%M-%S"))
...
Main = (function()
	...
	Main.Error = function(str)
		if rconsoleprint then
			rconsoleprint("VEX ERROR: "..tostring(str).."\n")
			wait(9e9)
		else
			error(str)
		end
	...
	Main.LoadModules = function()
		for i,v in pairs(Main.ModuleList) do
			...
			if not s then
				error(("FAILED LOADING %s CAUSE %s"):format(v, e))
			end
		end
	...
	Main.FetchAPI = function(bad, lol)
		...
		if Main.Elevated then
			...
			task.spawn(function()
			    wait(30)
			    if bad and not downloaded then bad() end
			    wait(60)
			    if lol and not downloaded then lol() end
			end)
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="Vex",TextColor3=Color3.new(1,1,1),TextSize=50,TextTransparency=1,}},
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="资源调试器",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="由Virtual Team修改",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "X" or "Vex"
	...
	Main.CreateMainGui = function()
		...
		openButton.MainFrame.BottomFrame.Information.MouseButton1Click:Connect(function()
		    ...
		    if ininfo then return end
		    ininfo = true
		    if not infointro then
		        infointro = Main.CreateIntro("......")
		        ininfo = true
		        coroutine.wrap(function()
		            while infointro do
		                for i,v in pairs(infos) do
		                    if not infointro then break end
		                    infointro.SetProgress(v, 1 / #infos * i)
		                    task.wait(duration)
		                end
		            end
		        end)()
		...
		-- Create Main Apps
		Main.CreateApp({Name = "资源", IconMap = Main.LargeIcons, Icon = "Explorer", Open = true, Window = Explorer.Window})

		Main.CreateApp({Name = "属性", IconMap = Main.LargeIcons, Icon = "Properties", Open = true, Window = Properties.Window})

		Main.CreateApp({Name = "脚本查看", IconMap = Main.LargeIcons, Icon = "Script_Viewer", Window = ScriptViewer.Window})

		local cptsOnMouseClick = nil
		Main.CreateApp({Name = "点击选择物体", IconMap = Main.LargeIcons, Icon = 6, OnClick = function(callback)
			if callback then
				local mouse = Main.Mouse
		...
		Main.CreateApp({Name = "保存", IconMap = Main.LargeIcons, Icon = "Watcher", Window = SaveInstance.Window})
		
		Main.CreateApp({Name = "选择多个", IconMap = Main.LargeIcons, Icon = 6, OnClick = function(callback)
			MultiSelect = callback
		end})
		
		Main.CreateApp({Name = "类名搜索", IconMap = Explorer.ClassIcons, Icon = 16.5, OnClick = function(callback)
			SearchClass = callback
		end})	
		
		Main.CreateApp({Name = "精准搜索", IconMap = Explorer.ClassIcons, Icon = 16.5, OnClick = function(callback)
			accurateSearch = callback
		end})	
	...
	Main.Init = function()
		...
		-- Load Lib
		local intro = Main.CreateIntro("初始化UI")
		...
		intro.SetProgress("正在获取Roblox版本",0.2)
		if Main.Elevated then
		...
		API = Main.FetchAPI(
		    function()
		        intro.SetProgress("正在获取API，网络不好", 0.4)
		    end,
		    function()
		        intro.SetProgress("正在获取API，网络非常差", 0.45)
		    end
		)
		Lib.FastWait()
		...
		-- Done
		intro.SetProgress("完成".. " 耗时" .. math.floor(tick() - old) .. "s",1)
		coroutine.wrap(function()
			Lib.FastWait(1.25)
			intro.Close()
		end)()
